...
const shouldRenderForBrowser = (props, browsers) => {
  ...
  const currentBrowser = browsers.find(browser => browser.isCurrentBrowser === true);
  if (currentBrowser && restrictedBrowsers.includes(currentBrowser.name)) return false;
  else if (currentBrowser && allowedBrowsers.includes(currentBrowser.name)) return true;
  else return false;
};
